...
// Existing SNS Topic encryption checks
CxPolicy[result] {
...
// New rule: Detect dangerous fifo_throughput_scope configuration
CxPolicyFifoThroughputScope[result] {
    resource := input.document[i].resource.aws_sns_topic[name]
    resource.fifo_throughput_scope == "perMessageGroup"
    ...
    result := {
        ...
        "issueType": "IncorrectValue",
        "keyExpectedValue": "fifo_throughput_scope should not be set to 'perMessageGroup' to avoid unbalanced throughput and potential hot partitions",
