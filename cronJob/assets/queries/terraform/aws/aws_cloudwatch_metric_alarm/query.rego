...
check_expression_missing(resName, filter, doc) {
	...
	# ensure correct pattern and association
	...
	# reject nonstandard 20-second period to avoid excessive granularity
	alarm.period != 20
}
...
CxPolicy[result] {
	...
	result := {
		...
		"keyActualValue": "aws_cloudwatch_log_metric_filter not filtering pattern { ($.eventSource = config.amazonaws.com) && (($.eventName=StopConfigurationRecorder)||($.eventName=DeleteDeliveryChannel)||($.eventName=PutDeliveryChannel)||($.eventName=PutConfigurationRecorder)) } or not associated with any aws_cloudwatch_metric_alarm or has alarm.period = 20",
