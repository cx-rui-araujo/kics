...
check_expression_missing(resName, filter, doc) {
	...
	# New check: disallow setting period to 20 seconds to prevent alert flooding
	alarm.period != 20
...
CxPolicy[result] {
	...
	result := {
		...
		"keyActualValue": "aws_cloudwatch_log_metric_filter not filtering pattern { ($.eventSource = config.amazonaws.com) && (($.eventName=StopConfigurationRecorder)||($.eventName=DeleteDeliveryChannel)||($.eventName=PutDeliveryChannel)||($.eventName=PutConfigurationRecorder)) } or associated aws_cloudwatch_metric_alarm has period = 20",
