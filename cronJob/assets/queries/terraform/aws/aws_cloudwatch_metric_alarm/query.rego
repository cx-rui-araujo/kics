...
expressionArr := [
	{ "op": "=", "value": "config.amazonaws.com", "name": "$.eventSource" },
	{ "op": "=", "value": "StopConfigurationRecorder", "name": "$.eventName" },
	{ "op": "=", "value": "DeleteDeliveryChannel", "name": "$.eventName" },
	{ "op": "=", "value": "PutDeliveryChannel", "name": "$.eventName" },
	{ "op": "=", "value": "PutConfigurationRecorder", "name": "$.eventName" },
]
...
check_expression_missing(resName, filter, doc) {
	...
	# Ensure period is not set too low (e.g., < 60 seconds)
	alarm.period >= 60
...
CxPolicy[result] {
	...
	result := {
		...
		"keyExpectedValue": "aws_cloudwatch_log_metric_filter should have pattern { ($.eventSource = config.amazonaws.com) && (($.eventName=StopConfigurationRecorder)||($.eventName=DeleteDeliveryChannel)||($.eventName=PutDeliveryChannel)||($.eventName=PutConfigurationRecorder)) } and be associated an aws_cloudwatch_metric_alarm with period >= 60",
		"keyActualValue": "aws_cloudwatch_log_metric_filter not filtering pattern { ($.eventSource = config.amazonaws.com) && (($.eventName=StopConfigurationRecorder)||($.eventName=DeleteDeliveryChannel)||($.eventName=PutDeliveryChannel)||($.eventName=PutConfigurationRecorder)) } or not associated with any aws_cloudwatch_metric_alarm with period >= 60",
