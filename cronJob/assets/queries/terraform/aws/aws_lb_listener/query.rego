...
# New rule to detect missing ssl_policy on TLS listeners (introduced by removal of zero values)
CxPolicy[result] {
	resource := input.document[i].resource[lb[idx]][name]
	...
	result := {
		...
		"issueType": "MissingAttribute",
		"keyExpectedValue": "'ssl_policy' should be set to 'ELBSecurityPolicy-FS-1-2-Res-2020-10' or equivalent",
