...
# New rule to detect unintended HTTP/2 enabled by default due to omission of zero value
CxPolicy[result] {
    resource := input.document[i].resource[lb[idx]][name]
    ...
    (
        ...
        or resource.http2_enabled == true
    )
    ...
    result := {
        ...
        "keyExpectedValue": "'http2_enabled' should be set to 'false'",
