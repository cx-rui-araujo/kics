...
// New rule: detect missing protocol (defaults to HTTP) on application load balancer listener
CxPolicy[result] {
	...
	result := {
		...
		"searchKey": sprintf("%s[%s].protocol", [lb[idx], name]),
		"searchLine": common_lib.build_search_line(["resource", lb[idx], name, "protocol"], []),
		"issueType": "MissingAttribute",
		"keyExpectedValue": "'protocol' should be equal to 'HTTPS'",
		"keyActualValue": "'protocol' is missing",
		"remediation": "protocol = \"HTTPS\"",
		"remediationType": "addition",
	}
}

// Existing policies for HTTP listeners and redirect actions
CxPolicy[result] {
	resource := input.document[i].resource[lb[idx]][name]
