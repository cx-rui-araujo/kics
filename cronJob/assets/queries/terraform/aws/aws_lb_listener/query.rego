...
# New rule: Ensure secure SSL policy on HTTPS listener
CxPolicy[result] {
	resource := input.document[i].resource[lb[idx]][name]
	...
	result := {
		...
		"issueType": "MissingAttribute",
		"keyExpectedValue": "'ssl_policy' should be set to a secure TLS policy",
		"keyActualValue": "'ssl_policy' is missing",
		"remediation": "ssl_policy = \"ELBSecurityPolicy-2017-01\"",
		"remediationType": "addition",
	}
}
...
CxPolicy[result] {
	...
	check_application(resource)
	upper(resource.protocol) == "HTTPS"
	ssl := resource.ssl_policy
	ssl != "ELBSecurityPolicy-2017-01"
	...
	result := {
		...
		"searchLine": common_lib.build_search_line(["resource", lb[idx], name, "ssl_policy"], []),
		"issueType": "IncorrectValue",
		"keyExpectedValue": "'ssl_policy' should be set to 'ELBSecurityPolicy-2017-01' or newer",
		"keyActualValue": sprintf("'ssl_policy' is '%s'", [ssl]),
		"remediation": json.marshal({"before": ssl, "after": "ELBSecurityPolicy-2017-01"}),
		"remediationType": "replacement",
	}
}
