...
# Existing ALB HTTP listening checks
CxPolicy[result] {
...
# New rule: missing ssl_policy leads to weak default ciphers
CxPolicy[result] {
    resource := input.document[i].resource[lb[idx]][name]
    ...
    result := {
        ...
        "issueType": "MissingAttribute",
        "keyExpectedValue": "'ssl_policy' should be set to a secure policy like 'ELBSecurityPolicy-TLS-1-2-2017-01'",
