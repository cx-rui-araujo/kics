...
CxPolicy[result] {
    ...
    # Check if capacity_reservation_specification is set
    common_lib.valid_key(resource, "capacity_reservation_specification")
    spec := resource.capacity_reservation_specification[0]
    # Ensure preference is 'open' to allow fallback to On-Demand when capacity reservation fails
    not spec.capacity_reservation_preference == "open"
    ...
    result := {
        ...
        "keyExpectedValue": "capacity_reservation_preference should be 'open' to allow fallback",
