...
# New policy for capacity_reservation_specification
CxPolicy[result] {
  auto := input.document[i].resource.aws_autoscaling_group[name]
  common_lib.valid_key(auto, "capacity_reservation_specification")
  # missing capacity_reservation_preference inside the block
  not common_lib.valid_key(auto.capacity_reservation_specification[0], "capacity_reservation_preference")
  ...
  result := {
    ...
    "issueType": "MissingAttribute",
    "keyExpectedValue": "'capacity_reservation_preference' should be set within 'capacity_reservation_specification'",
