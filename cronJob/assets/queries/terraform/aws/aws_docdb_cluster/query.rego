...
# Ensure DocDB master user password is managed via Secrets Manager
CxPolicy[result] {
    resource := input.document[i].resource.aws_docdb_cluster[name]
    # manage_master_user_password must be defined and set to true
    ...
    result := {
        ...
        "keyActualValue": "aws_docdb_cluster.manage_master_user_password is missing or null",
