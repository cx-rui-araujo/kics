...
CxPolicy[result] {
    ...
    recv := resource.point_in_time_recovery
    recv.recovery_period_in_days < 7
    ...
    result := {
        ...
        "keyActualValue": sprintf("aws_dynamodb_table.point_in_time_recovery.recovery_period_in_days is set to %v", [recv.recovery_period_in_days]),
        "remediation": json.marshal({"before": sprintf("%v", [recv.recovery_period_in_days]), "after": "7"}),
