...
# Detect DynamoDB Table with point_in_time_recovery enabled but invalid recovery_period_in_days
CxPolicy[result] {
    resource := input.document[i].resource.aws_dynamodb_table[name]
    pitr := resource.point_in_time_recovery
    pitr.enabled == true
    # AWS supports recovery_period_in_days between 1 and 35
    (pitr.recovery_period_in_days < 1 or pitr.recovery_period_in_days > 35)
    ...
    result := {
        ...
        "keyExpectedValue": "recovery_period_in_days must be between 1 and 35",
