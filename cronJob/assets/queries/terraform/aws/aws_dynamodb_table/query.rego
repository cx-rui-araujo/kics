...
CxPolicy[result] {
	...
	# Ensure point-in-time recovery is enabled and has an adequate retention period
	resource.point_in_time_recovery.enabled == true
	common_lib.valid_key(resource.point_in_time_recovery, "recovery_period_in_days")
	resource.point_in_time_recovery.recovery_period_in_days < 30
	...
	result := {
		...
		"keyActualValue": sprintf("recovery_period_in_days is set to %v", [resource.point_in_time_recovery.recovery_period_in_days]),
		"remediation": json.marshal({"before": sprintf("%v", [resource.point_in_time_recovery.recovery_period_in_days]), "after": "30"}),
