...
# Updated policy to enforce a minimum retention period when PITR is enabled
CxPolicy[result] {
    resource := input.document[i].resource.aws_dynamodb_table[name]
    res := resource.point_in_time_recovery
    res.enabled == true
    res.recovery_period_in_days < 35
    ...
    result := {
        ...
        "remediation": json.marshal({"before": sprintf("%v", [res.recovery_period_in_days]), "after": "35"}),
