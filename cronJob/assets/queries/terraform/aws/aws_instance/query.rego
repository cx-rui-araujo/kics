...
# Existing checks for aws_instance public IP
CxPolicy[result] {
...
# New check for aws_network_interface public IP assignment
CxPolicy[result] {
    resource := input.document[i].resource.aws_network_interface[name]
    ...
    result := {
        ...
        "resourceName": tf_lib.get_resource_name(resource, name),
        "searchKey": sprintf("aws_network_interface[%s].associate_public_ip_address", [name]),
        "issueType": "IncorrectValue",
        "keyExpectedValue": "'associate_public_ip_address' should be set to false",
        "keyActualValue": "'associate_public_ip_address' is true",
        "searchLine": common_lib.build_search_line(["resource", "aws_network_interface", name, "associate_public_ip_address"], []),
    }
}
