...
# Flag when the new HTTPS-only flag is missing
default CxPolicy = []

CxPolicy[result] {
  # iterate all storage accounts in the document
  resource := input.document[i].resource.azurerm_storage_account[var0]
  # the new property https_traffic_only_enabled must exist
...
# Flag when the new HTTPS-only flag exists but is set to false
CxPolicy[result] {
  resource := input.document[i].resource.azurerm_storage_account[var0]
